    ...
    if (req.query.id) {
        if (findRoom(req.query.id)) {
            if (findRoom(req.query.id).users < 2) {
            	if(findRoom(req.query.id).users == 0){
            		var user = 1;
            		isOnline[req.query.id][1] = setTimeout(function() {
            			findRoom(req.query.id).users -=1;
            		}, 5000);
            	}
            	else if(findRoom(req.query.id).users == 1){
            		var user = 2;
            		isOnline[req.query.id][2] = setTimeout(function() {
            			findRoom(req.query.id).users -=1;
            		}, 5000);
            	}
            	findRoom(req.query.id).users += 1;
                res.render('game', { room: findRoom(req.query.id), user: user});
            } else {
                res.render('rooms', { rooms: rooms });
            }
        } else {
...
router.get('/getData', function(req, res, next) {
    ...
    if(req.query.user){
    	clearTimeout(isOnline[id][req.query.user])
    	isOnline[id][req.query.user] = setTimeout(function() {
            findRoom(req.query.id).users -=1;
        }, 5000);
    }
});
